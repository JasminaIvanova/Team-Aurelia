...
namespace Aurelia.App.Areas.Identity.Pages.Account
{
    ...
    public class ExternalLoginModel : PageModel
    {
        ...
        public ExternalLoginModel(
            ...
            IEmailSender emailSender, ApplicationDbContext aureliaDB)
        {
        ...
        public async Task<IActionResult> OnPostConfirmationAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                ExternalLoginInfo infoG = await _signInManager.GetExternalLoginInfoAsync();

                await _userStore.SetUserNameAsync(user, Input.Email, CancellationToken.None);
                await _emailStore.SetEmailAsync(user, Input.Email, CancellationToken.None);
                user.FirstName = infoG.Principal.FindFirstValue(ClaimTypes.GivenName) ?? info.Principal.FindFirstValue(ClaimTypes.Name);
                user.SecondName = "";
                user.LastName = infoG.Principal.FindFirstValue(ClaimTypes.Surname);
        ...
        private AureliaUser CreateUser()
        {
            try
            {
            ...
            catch
            {
                throw new InvalidOperationException($"Can't create an instance of '{nameof(AureliaUser)}'. " +
                    $"Ensure that '{nameof(AureliaUser)}' is not an abstract class and has a parameterless constructor, or alternatively " +
